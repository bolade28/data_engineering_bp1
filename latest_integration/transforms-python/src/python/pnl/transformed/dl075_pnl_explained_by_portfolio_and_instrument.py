...
def dl075_transformed(df, ref_data):
...
@transform_df(
    Output("/BP/IST-IG-DD/data/technical/pnl/transformed/dl075_pnl_explained_by_portfolio_and_instrument"),
    df=Input("/BP/IST-IG-DD/data/technical/pnl/current/endur/dl075-pnl_explained_by_portfolio_and_instrument"),
...
def dtd_calc(df, ref_data):
    ...
    Version: T1
    This function:
        Transforms dl075 current
        Renames the columns of the typed dataframe
        Reorders the dataframe based on its respective output schema

    Args:
        params1 (dataframe): current dl075

    Returns:
        dataframe: transformed dl075
    """
    df_out = dl075_transformed(df, ref_data)
